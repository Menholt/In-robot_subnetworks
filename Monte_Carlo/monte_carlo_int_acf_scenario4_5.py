# -*- coding: utf-8 -*-
"""
Created on Thu May 26 13:43:28 2022
...
def monte_int_ss(x_t1, x_t2, tau, displacement_t1, displacement_t2, grf, delta, stepsize, int_samples, r, height, width, min_dist, vel, c, fc, N_sg, n_nodes):
    ...
    cov_ss += (special.jv(0, 2*np.pi*tau*vel*fc/c)**2*0.7+1)/2* eta_
    cov_ss += 1/(4) * kappa_alpha(x_t1, x_t2, displacement_t1, displacement_t1, r, int_samples, grf, delta, stepsize, height, width)
    return cov_ss
...
def monte_int_cucu(x_t1, x_t2, tau, grf, delta, stepsize, r, height, width, min_dist, vel, c, fc, N_sg, n_nodes):
    ...
    for i in range(len(x_t1)-1):
        ...
        for j in range(len(x_t1)-1):
            if j!=i:
                cov_cucu += 1/4 *pathloss(x_t1[0] - x_t1[i+1])*i_f.shadowing(grf, delta, x_t1[i+1], x_t1[0], stepsize, height, width)\
